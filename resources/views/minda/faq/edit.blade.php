...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('minda.faq.update', $faq->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <textarea name="jawaban" rows="6" required
                    ...
                    placeholder="Tulis jawaban FAQ">{{ old('jawaban', $faq->jawaban) }}</textarea>
                @error('jawaban')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-5">
                ...
                <div>
                    ...
                    <label class="flex items-center gap-3 mt-3 cursor-pointer">
                        <input type="checkbox" name="aktif" value="1" {{ old('aktif', $faq->aktif) ? 'checked' : '' }}
