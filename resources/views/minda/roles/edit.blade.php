...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="mb-5 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700">
            <strong>Info:</strong> Role ini digunakan oleh {{ $role->admins()->count() }} admin. Perubahan permission akan berlaku langsung untuk semua admin tersebut.
        </div>
        @endif

        <form action="{{ route('minda.roles.update', $role->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <label class="flex items-center gap-2.5 p-2.5 rounded-lg border border-purple-100 bg-white cursor-pointer hover:bg-purple-50 transition has-[:checked]:border-purple-400 has-[:checked]:bg-purple-100">
                            <input type="checkbox" name="allowed_lembaga[]" value="{{ $lmb->id }}"
                                {{ in_array($lmb->id, old('allowed_lembaga', $role->allowed_lembaga ?? [])) ? 'checked' : '' }}
