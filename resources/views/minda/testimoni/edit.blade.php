...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('minda.testimoni.update', $testimoni->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <textarea name="isi" rows="5" required
                    ...
                    placeholder="Tuliskan testimoni...">{{ old('isi', $testimoni->isi) }}</textarea>
                @error('isi')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-5">
                ...
                @if($testimoni->foto)
                    <div class="mb-3 flex items-center gap-3">
                        <img src="{{ asset('storage/' . $testimoni->foto) }}" alt="{{ $testimoni->nama }}" class="w-16 h-16 rounded-full object-cover">
                ...
                <p class="text-xs text-gray-400 mt-1">Kosongkan jika tidak ingin mengubah foto. Maks 2MB.</p>
                @error('foto')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-5">
                <div>
                    ...
                    <input type="number" name="urutan" value="{{ old('urutan', $testimoni->urutan) }}" min="0"
                ...
                <div>
                    ...
                    <label class="flex items-center gap-3 mt-3 cursor-pointer">
                        <input type="checkbox" name="aktif" value="1" {{ old('aktif', $testimoni->aktif) ? 'checked' : '' }}
