...
@section('page-title', 'Kelola Banner Halaman Utama')

@section('content')
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <p class="text-gray-600">Kelola foto/banner carousel di halaman utama</p>
        <a href="{{ route('minda.banner.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-medium transition flex items-center gap-2">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden {{ !$item->aktif ? 'opacity-50' : '' }}">
                    ...
                    <div class="relative aspect-[16/9] bg-gray-100">
                        @if(str_starts_with($item->gambar, 'images/'))
                            <img src="{{ asset($item->gambar) }}" alt="{{ $item->judul ?? 'Banner' }}" class="w-full h-full object-cover">
                        @else
                            <img src="{{ asset('storage/' . $item->gambar) }}" alt="{{ $item->judul ?? 'Banner' }}" class="w-full h-full object-cover">
                        @endif
                    ...
                    {{-- Info --}}
                    <div class="p-4">
                        <h3 class="font-semibold text-gray-800 text-sm truncate">{{ $item->judul ?? 'Tanpa judul' }}</h3>
    ...
    @else
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <a href="{{ route('minda.banner.create') }}" class="text-blue-600 hover:text-blue-800 font-medium">+ Tambah banner pertama</a>
        </div>
    @endif
</div>
@endsection
