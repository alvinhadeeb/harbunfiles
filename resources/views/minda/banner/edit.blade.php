...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('minda.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-5">
                <label class="block text-gray-700 font-semibold mb-2">Judul / Keterangan</label>
                <input type="text" name="judul" value="{{ old('judul', $banner->judul) }}"
                    ...
                    placeholder="Opsional, contoh: Hari Pahlawan 2024">
                @error('judul')
            ...
            <div class="mb-5">
                <label class="block text-gray-700 font-semibold mb-2">Gambar Banner</label>
                {{-- Current Image --}}
                <div class="mb-3">
                    @if(str_starts_with($banner->gambar, 'images/'))
                        <img src="{{ asset($banner->gambar) }}" alt="{{ $banner->judul ?? 'Banner' }}" class="w-full rounded-lg border border-gray-200" style="max-height: 250px; object-fit: cover;">
                    @else
                        <img src="{{ asset('storage/' . $banner->gambar) }}" alt="{{ $banner->judul ?? 'Banner' }}" class="w-full rounded-lg border border-gray-200" style="max-height: 250px; object-fit: cover;">
                    @endif
                ...
                <p class="text-xs text-gray-400 mt-1">Kosongkan jika tidak ingin mengubah gambar. Maks 5MB.</p>
                @error('gambar')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                {{-- Preview --}}
                <div id="preview-container" class="mt-3 hidden">
                    ...
                    <p class="text-xs text-green-600 mt-1">Gambar baru (preview)</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-5">
                ...
                <div>
                    ...
                    <label class="flex items-center gap-3 mt-3 cursor-pointer">
                        <input type="checkbox" name="aktif" value="1" {{ old('aktif', $banner->aktif) ? 'checked' : '' }}
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-200">
                ...
                <a href="{{ route('minda.banner.index') }}" class="text-gray-600 hover:text-gray-800 px-4 py-2.5 font-medium">
                    Batal
                </a>
            </div>
        </form>
